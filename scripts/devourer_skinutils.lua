...
AddSkin("devourer_pack", "devourer_cats", {
    type = "base",                                         --类型，base就行
    rarity = "Elegant",                                    --稀有度，会给皮肤字体加不同颜色
    init_fn = function(inst)                               --皮肤初始化函数
        inst.components.inventoryitem.atlasname = "images/inventoryimages/devourer_cats.xml"
        basic_init_fn(inst, "devourer_cats", "devourer_pack") --prefabskin.lua提供的函数，还挺简便的
    end,
    clear_fn = function(inst)                              --不再使用该皮肤时
        inst.components.inventoryitem.atlasname = nil
        basic_clear_fn(inst, "devourer_pack")
    end
